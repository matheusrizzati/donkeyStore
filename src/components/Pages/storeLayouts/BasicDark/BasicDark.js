...
function BasicDark(props){
    ...
    const notify = (item) => toast.success(`${item} adicionado ao carrinho`, {position: "bottom-right", autoClose: 3000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, theme: "dark"})
    ...
    return(
        ...
        <main className={styles.main}>
            {produtos && produtos.length !== 0 && produtos.map(item => (
                ...
                <button className={styles.addCart} onClick={() => {addItemToCart(item); notify(item.name)}}>Comprar</button>
                </div>
            ))}
        </main>
        </>
